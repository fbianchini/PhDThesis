...
class lcdm():
	...
	def __init__(self, params): #omr=0.0, omb=0.05, omc=0.25, oml=0.7, H0=70. ):
		...
		pars.set_cosmology(H0=self.H0, ombh2=self.omegab*self.h**2, omch2=self.omegac*self.h**2, omk=0)
		pars.InitPower.set_params(ns=params['scalar_index'])
		...
		self.pkz = camb.get_matter_power_interpolator(pars, nonlinear=True, 
		    hubble_units=True, k_hunit=True)#, kmax=max_kappa)
	...
	def power_spectra_clustering_at_z_and_l(self, z, ell, dndz1, dndz2=None, b1=1, b2=1):
		if np.isscalar(z) or (np.size(z) == 1):
			if dndz2 is None:
			    return (self.H_z(z) * (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_cl(z, dndz1, b=b1)**2 * self.pkz.P(x, ell/(self.x_z(z)*self.h))
			else:
			    return (self.H_z(z) * (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_cl(z, dndz1, b=b1) * self.w_cl(z, dndz2, b=b2) * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
	...
	def power_spectra_crossterm_at_z_and_l(self, z, ell, dndz1, dndz2, b1=1, alpha2=1):
		...
		if np.isscalar(z) or (np.size(z) == 1):
			return (self.H_z(z)* (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_cl(z, dndz1, b=b1)*self.w_mu(z, dndz2, alpha=alpha2) * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
	...
	def power_spectra_magnification_at_z_and_l(self, z, ell, dndz1, dndz2=None, alpha1=1, alpha2=1):
		if np.isscalar(z) or (np.size(z) == 1):
		    if dndz2 is None:
		        return (self.H_z(z)* (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_mu(z, dndz1, alpha=alpha1)**2 * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		    else:
		        return (self.H_z(z)* (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_mu(z, dndz1, alpha=alpha1)*self.w_mu(z, dndz2, alpha=alpha2) * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
	...
	def power_spectra_kg_at_z_and_l(self, z, ell, dndz, b=1):
		if np.isscalar(z) or (np.size(z) == 1):
			return (self.H_z(z) * (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_cl(z, dndz, b=b) * self.w_kappa_cmb(z) * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
	...
	def power_spectra_kmu_at_z_and_l(self, z, ell, dndz): # !!! WITHOUT THE (\alpha - 1) FACTOR !!!
		if np.isscalar(z) or (np.size(z) == 1):
			return (self.H_z(z) * (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_kappa_cmb(z) * self.w_mu(z, dndz) * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
	...
	def power_spectra_gg_tot_at_z_and_l(self, z, ell, dndz, b=1, alpha=1):
		if np.isscalar(z) or (np.size(z) == 1):
		    return (self.H_z(z) * (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * (self.w_cl(z, dndz, b=b) + (alpha-1)*self.w_mu(z, dndz, alpha=alpha))**2 * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
			return [ self.power_spectra_gg_tot_at_z_and_l(tz, ell, dndz, b=b, alpha=alpha) for tz in z ]

	def power_spectra_gg_tot(self, dndz, b=1, alpha=1, lmax=2500, dz=0.05, nell=20):
		cl_spline   = []

		l_log = np.logspace(np.log10(2),np.log10(lmax),nell)
		l_log = np.unique(l_log.astype(int))
		good  = np.where(l_log<3)
		l_log = l_log[np.where(l_log>20)]
		l_log = np.insert(l_log,0,2)
	...
	def power_spectra_kg_tot_at_z_and_l(self, z, ell, dndz, b=1, alpha=1):
		if np.isscalar(z) or (np.size(z) == 1):
		    return (self.H_z(z) * (u.km).to(u.Mpc))/(self.x_z(z)**2 * const.c.to('Mpc/s').value) * self.w_kappa_cmb(z) * (self.w_cl(z, dndz, b=b) + (alpha-1)*self.w_mu(z, dndz, alpha=alpha)) * self.pkz.P(z, ell/(self.x_z(z)*self.h))
		else:
			return [ self.power_spectra_kg_tot_at_z_and_l(tz, ell, dndz, b=b, alpha=alpha) for tz in z ]

	def power_spectra_kg_tot(self, dndz, b=1, alpha=1, lmax=2500, dz=0.05, nell=20):
		cl_spline   = []
